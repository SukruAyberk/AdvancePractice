...
public class Q10 {
	 ...
	 */
        ...
        String gizlenmisIsim = isim.substring(0, 1) + isim.substring(1, isim.length()).replaceAll("\\D", "*");
        String gizlenmisSoyisim = soyisim.substring(0, 1) + soyisim.substring(1, soyisim.length()).replaceAll("\\D", "*");

        if (kKNo.length() < 16) {
            System.out.println("Geçersiz kredi kartı numarası!");
        } else {
            System.out.println("Ad: " + gizlenmisIsim + " Soyad: " + gizlenmisSoyisim + "Kart No: " + "**** **** **** " + kKNo.substring(12));
        }
